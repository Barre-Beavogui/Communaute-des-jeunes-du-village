import { ArrowUpRight, FolderKanban, MapPin, Search, UsersRound } from 'lucide-react';
import { useMemo, useState } from 'react';
import { Link } from 'wouter';
import { getGetMembersSummaryQueryKey, getListProfilesQueryKey, useGetMembersSummary, useListProfiles } from '@workspace/api-client-react';
...
export default function HomePage() {
  const profilesQuery = useListProfiles({ query: { queryKey: getListProfilesQueryKey() } });
  ...
  const filteredProfiles = profiles.filter((profile) => {
    ...
    const matchesSearch = !query || `${profile.name} ${profile.neighborhood} ${profile.activities.join(' ')}`.toLowerCase().includes(query);
    const matchesActivity = activity === 'Tous' || profile.activities.includes(activity);
    ...
    <div className="space-y-9">
      <section className="vj-enter relative overflow-hidden rounded-[28px] bg-foreground px-6 py-9 text-background shadow-[0_20px_45px_hsl(300_18%_18%/.14)] sm:px-10 sm:py-12 lg:px-14 lg:py-14">
        ...
        <div className="absolute bottom-8 right-10 hidden rotate-6 rounded-2xl border border-background/20 bg-background/10 px-4 py-3 backdrop-blur-sm sm:block">
          <p className="font-mono text-[10px] uppercase tracking-widest text-background/60">Ici, on partage</p>
          <p className="mt-1 text-sm font-bold">des idées, pas des likes.</p>
        </div>
      </section>

      <section className="grid gap-4 sm:grid-cols-3" aria-label="Les chiffres du village">
        <div className="vj-enter vj-enter-delay-1 rounded-2xl border border-border bg-card p-5 shadow-sm">
          <div className="flex items-center justify-between"><UsersRound className="h-5 w-5 text-primary" /><span className="font-mono text-[10px] text-muted-foreground">01</span></div>
          <p className="mt-6 text-3xl font-extrabold tracking-[-.06em]" data-testid="text-total-members">{summary.totalMembers}</p>
          <p className="mt-1 text-xs font-semibold text-muted-foreground">visages dans le village</p>
        </div>
        <div className="vj-enter vj-enter-delay-2 rounded-2xl border border-border bg-card p-5 shadow-sm">
          <div className="flex items-center justify-between"><FolderKanban className="h-5 w-5 text-secondary" /><span className="font-mono text-[10px] text-muted-foreground">02</span></div>
          <p className="mt-6 text-3xl font-extrabold tracking-[-.06em]" data-testid="text-active-projects">{summary.activeProjects}</p>
          <p className="mt-1 text-xs font-semibold text-muted-foreground">projets qui prennent vie</p>
        </div>
      ...
      <section id="visages" className="scroll-mt-28 space-y-5">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <p className="font-mono text-[10px] font-bold uppercase tracking-[.18em] text-primary">La place centrale</p>
            <h2 className="vj-display mt-1 text-4xl leading-none sm:text-5xl">Qui est dans le coin ?</h2>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {activities.slice(0, 9).map((item) => (
            <button type="button" key={item} onClick={() => setActivity(item)} className={`shrink-0 rounded-full border px-3.5 py-2 text-xs font-bold ${activity === item ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card text-muted-foreground hover:border-primary/50 hover:text-foreground'}`} data-testid={`button-filter-${item.toLowerCase()}`}>
              {item}
            </button>
          ))}
        </div>
        {profilesQuery.isLoading ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((item) => <div key={item} className="h-56 animate-pulse rounded-2xl bg-muted" data-testid={`skeleton-member-${item}`} />)}
          </div>
        ) : filteredProfiles.length ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredProfiles.map((profile, index) => (
              <Link href={`/membre/${profile.id}`} key={profile.id} className={`group relative overflow-hidden rounded-2xl border border-border bg-card p-5 shadow-sm hover:-translate-y-1 hover:border-primary/40 hover:shadow-md ${index === 0 ? 'lg:col-span-2' : ''}`} data-testid={`card-member-${profile.id}`}>
          ...
          <div className="rounded-2xl border border-dashed border-border bg-card px-6 py-14 text-center" data-testid="empty-members">
            <p className="vj-display text-3xl">Personne dans ce coin… pour l’instant.</p>
            <p className="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">Essayez une autre recherche ou invitez un visage familier à rejoindre le village.</p>
            <Link href="/inscription" className="mt-6 inline-flex rounded-full bg-primary px-4 py-2.5 text-xs font-bold text-primary-foreground" data-testid="link-empty-join">Proposer une inscription</Link>
          </div>
        )}
        {profilesQuery.isError && <p className="text-xs font-semibold text-muted-foreground" data-testid="status-members-fallback">Le serveur se réveille doucement — voici un aperçu du village.</p>}
      </section>
    </div>
  );
}
